function [ chem ] = loadSafetyInfo( pathSafetyInfo, pathCrw, chem)
...
raw = xlsReadPretty(pathSafetyInfo, chmLen, 'NoLiteralQuote');
...
for c=1:chmLen
  ...
  if c==1
      ...
      [indVar indVal indUnit indField] = findColNums(raw{1});
      fnd = 1;
      last = size(raw{1}, 1);
      for ih = 2:last
          if (ischar(raw{1}{ih, indField}) || ~isnan(raw{1}{ih, indField}))
              fields{fnd} = raw{1}{ih, indField};
              fnd = fnd+1;
          end
      end
  end
  rawID = raw{c}(3, indVal);

  creal = find(strcmp({chem.ID}, rawID));

  if ~isempty(creal)
      chem(creal).safetyInfo = raw{c};
  end
